...
# Documentation:
# @raycast.description Backup SD card in two SSD
# @raycast.author Adrien Le Falher
# @raycast.authorURL www.adrienlefalher.com

# Raycast Required Parameters
# @raycast.schemaVersion 1
# @raycast.title Launch Photo Backup (CLI)
# @raycast.mode compact
# @raycast.packageName Photo Tools

# Raycast Optional Parameters
# @raycast.icon 💻
# @raycast.description Launches the photo backup tool in interactive CLI mode in a new terminal window.
...
# 1. Change to the script's directory (important for relative paths like config file)
# 2. Execute the script using the Python from the specified virtual environment
COMMAND_TO_RUN="cd '$SCRIPT_DIR' && '$VENV_PYTHON' '$SCRIPT_PATH'"
...
# The window will remain open after the script finishes so you can see output
osascript <<EOF
...
echo "Photo Backup CLI launch command sent to Terminal."
exit 0